...
M=H*diagonalMatrix(R,4,4,{l_(5,1),l_(5,2),l_(5,3),l_(5,3)})*inverse(H)
...
qbar=time H4*qq;
-- used 8.51562 seconds

"F84_4leaves_tensor_id.txt" << toString qbar << endl << close
...
qq2=matrix qq2;
qbar2=time H4*qq2;
-- used 8.21875 seconds
"F84_4leaves_tensor_id_1324.txt" << toString qbar2 << endl << close
...
qbar3=time H4*qq3;
-- used 8.1875 seconds
"F84_4leaves_tensor_id_1423.txt" << toString qbar3 << endl << close
...
(p_3^3*p_4^3)*qbar_(position(S,j->j==(3,3,3,3)),0)-(p_1^3*p_2^3)*qbar_(position(S,j->j==(4,4,4,4)),0)
qbar_(position(S,j->j==(3,3,3,3)),0)
sub(((coefficients qbar_(position(S,j->j==(3,3,3,3)),0))_1)_(0,0),K)/sub(((coefficients qbar_(position(S,j->j==(4,4,4,4)),0))_1)_(0,0),K)
sub(((coefficients qbar_(position(S,j->j==(3,3,3,3)),0))_1)_(1,0),K)/sub(((coefficients qbar_(position(S,j->j==(4,4,4,4)),0))_1)_(1,0),K)
sub(((coefficients qbar_(position(S,j->j==(3,3,3,3)),0))_1)_(2,0),K)/sub(((coefficients qbar_(position(S,j->j==(4,4,4,4)),0))_1)_(2,0),K)
--no linear relations between q3333 and q4444
(p_3*p_4^3)*qbar_(position(S,j->j==(2,2,3,3)),0)-(p_1*p_2)*qbar_(position(S,j->j==(2,2,4,4)),0)

qbar_(position(S,j->j==(2,3,3,3)),0)
qbar_(position(S,j->j==(2,4,4,4)),0)
...
aux=time sub(flattP,toList apply(1..5,i->l_(i,4)=>l_(i,3)));
RG=K[l_(1,1)..l_(5,3)]
flattP=sub(aux,RG);
--rk 0
...
auxpbar=time sub(pbar,toList apply(1..5,i->l_(i,4)=>l_(i,3)));
pbar=sub(auxpbar,RG);
pbar_(position(S,j->j==(2,2,3,3)),0)
pbar_(position(S,j->j==(2,2,4,4)),0)
"F84_4leaves_tensor.txt" << toString pbar << endl << close
